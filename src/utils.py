    ...
    if data_api_response.status_code != 200:
        ...
        if "detail" in error_response and "message" in error_response["detail"]:
            ...
            if "Invalid signature" in error_message:
                print(
                    (
                        "You need to re-authenticate and generate a new token\n"
                        f"Navigate to {BETTEROMICS_URL} and log in"
                    )
                )
...
BETTEROMICS_API_URL = (
    "https://api.public-demo.betteromics.com/api"  # used to make API calls
)
